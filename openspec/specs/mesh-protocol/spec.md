...
## Purpose
Wire protocol for mesh messages. Defines compact JSON schema, message types, deduplication, and encoding rules for fitting within LoRa's 200-byte payload budget.
...
Location messages (`loc` type) SHALL encode latitude and longitude as fixed-point integers (6 decimal places x 1,000,000) to save bytes compared to float strings.
